...
document.addEventListener(/*'LOADED_EVENT_GOES_HERE'*/"DOMContentLoaded", () => {
  ...
  calculatorNumbers.forEach( key => {
    key.addEventListener( "click", event => clickNumber( event ) ) } );
  ...
  const calculatorDecimal = document.querySelectorAll(".decimal");
  calculatorDecimal.forEach( key => {
    key.addEventListener( "click", event => clickNumber( event ) );
  })
  ...
  const clearDisplay = document.querySelector(".all-clear");
  ...
  calculatorOperators.forEach( key => {
    key.addEventListener( "click", event => clickOperator( event ) );
  } );
  ...
  calculate.addEventListener( "click", () => performOperation() );
});
